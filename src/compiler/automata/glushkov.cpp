...
namespace aion::automata
{
    ...
    GlushkovVisitor::GlushkovVisitor(frontend::RegexMetadata& _meta, core::CompilationContext& _ctxt)
    ...
    {
        const std::uint16_t num_unions = node.options.size();
    ...
    {
        const std::uint16_t num_concats = node.sequence.size();
