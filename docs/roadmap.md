...
Currently, Aion does no type checking on the predicates provided by the user. This causes all errors in the predicates to be forwarded to the generated C++ code. While this is acceptable for the current prototype, ideally there should be a type checking pass over the AST before glushkov
construction. This is especially important for the next point.

### Basic Arithmetic Support

In real streams, we might not only want to compare event fields, but also
use basic arithmetic operations(+, -, *) to compare the fields. This would certainly
